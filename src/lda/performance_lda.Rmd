...
wordclouds <- function(model, topic) {
  cloud(model, topic = topic, max.words = 40, scale = c(2, .5))
}
...
```
Note about different types of word outputs from STM vignette: "The function by default prints several different types of word profiles, including highest probability words and FREX words. FREX weights words by their overall frequency and how exclusive they are to the topic... Lift weights words by dividing by their frequency in other topics, therefore giving higher weight to words that appear less frequently in other topics...Similar to lift, score divides the log frequency of the word in the topic by the log frequency of the word in other topics."
